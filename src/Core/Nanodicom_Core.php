...
namespace mmerlijn\dicom\Core;
use Exception;
use mmerlijn\dicom\Nanodicom;
use mmerlijn\dicom\Tools\Dicom_Anonymizer;
use mmerlijn\dicom\Tools\Dicom_Dumper;
use mmerlijn\dicom\Tools\Dicom_Getter;
use mmerlijn\dicom\Tools\Dicom_Pixeler;
use mmerlijn\dicom\Tools\Dicom_Simple;
...
abstract class Nanodicom_Core {
	...
	*/
	 ...
	 *
	 * @param   mixed $location blob or location of the file
	 * @param string $name name of the tool to load
	 * @param string $type type of data passed
	 * @return  Dicom_Anonymizer|Dicom_Dumper|Dicom_Getter|Dicom_Pixeler|Dicom_Simple A Tool
	 */
	public static function factory(mixed $location, string $name = 'Simple', string $type = 'file')
	{
		// Add the Dicom prefix
        return match($name){
            'Dumper' => new Dicom_Dumper($location, $name, $type),
            'Getter' => new Dicom_Getter($location, $name, $type),
            'Anonymizer' => new Dicom_Anonymizer($location, $name, $type),
            'Pixeler' => new Dicom_Pixeler($location, $name, $type),
            default => new Dicom_Simple($location, $name, $type)
        };
	 ...
	 */
	public static function decode_transfer_syntax($transfer_syntax): array
    {
	...
	protected static string $_read_int  = '_read_int_32';
	
	// Define which function to write integers based on size of integer
	protected static string $_write_int = '_write_int_32';

	// In case a string was given for vr list, we force to load dictionaries
	protected bool $_force_load_dictionary = FALSE;
	...
	public bool $has_dicom_preamble = FALSE;
	 ...
	 */
	public int $status = self::INITIAL;

	// Flag to know if file has been parsed
	protected bool $_is_parsed = FALSE;
	
	// Preamble
	protected $_preamble;

	// Location of file or 'blob'
	protected mixed $_location;

	// Metagroup last byte
	protected int $_meta_group_last_byte = -1;
	
	// Length of the file or blob
	protected int $_file_length;

	// The current pointer in the blob
	protected int $_current_pointer = 0;

	// The meta information group length
	protected int|null $_meta_information_group_length = NULL;

	// In case we want to read only certain fields
	protected array $_vr_reading_list = [];
	
	// Transfer Syntax. Defaults to Implicit Little Endian
	protected string $_transfer_syntax = self::IMPLICIT_VR_LITTLE_ENDIAN;
	...
	protected int $_counted_elements = 0;

	// Flag to know if Group 8 has been found
	protected bool $_found_group_8 = FALSE;
	
	// Callback to check if we need to stop after certain tags are read
	protected string $_check_list_function = '_dummy';
	
	// Callback to check deflate
	protected string $_check_deflate_function = '_check_deflate';

	// Callback to check proper endian
	protected string $_check_proper_endian_function = '_check_proper_endian';
	
	// Number of retries to read oversized lenghts
	protected int $_oversize_retries = 0;
	 ...
	 *
	 * @param   mixed $location file blob or location of the file
	 * @param string $name the name of the tool created
	 * @param string $type type of first parameter
	 * @return  Nanodicom
	 */
	public function __construct(mixed $location, string $name, string $type)
	{
	 ...
	 *
	 * @param string $name tag element name
	 * @return  mixed
	 */
	 ...
	 *
	 * @param string $name name of the tool to extend
	 * @return  Nanodicom_Core
     */
	public function extend(string $name):Nanodicom_Core
	{
	 ...
	 */
	public function profiler_diff(string $name, $units = 'ms'): float|int
    {
		$diff = 0;
	 ...
	 */
	public function last_error(): mixed
    {
	 ...
	 * returned status.
	 *
	 * @param boolean $allow_partial set to TRUE to allow partially read files
	 * @return  boolean	 true if file was parsed (depends on parameter to check partial reads), false otherwise
	 */
	public function is_dicom(bool $allow_partial = FALSE):bool
	{
		// Parse the file
		$this->parse();
	 ...
	 */
	public function get_transfer_syntax(): string
    {
		// Parse the file
		$this->parse();
	 ...
	 *
	 */
	public function flush():Nanodicom_Core
	{
	 ...
	 * @return	string	 the binary value
	 */
	protected function _write_int_32($value, $bytes, $endian, $length, $sign = self::UNSIGNED): string
    {
		if ($length == 0) return '';
	 ...
	 * Reads a float
	 *
	 * @param integer $bytes number of bytes to read per float
	 * @param integer $length total length to read
	 * @return	mixed	 a float or an array of floats
	 */
	protected function _read_float(int $bytes, int $length): mixed
    {
		// In case the file said 0 for the length?
	 ...
	 * Writes a float
	 *
	 * @param integer|array $value the new value
	 * @param integer $bytes the bytes needed per integer
	 * @param integer $length the number of bytes to read
	 * @return	string	 the binary value
	 */
	protected function _write_float(int|array $value, int $bytes, int $length): string
    {
		if ($length == 0) return '';
	 ...
	 *
	 * @param integer $position the new position, defaults to beginning of file
	 * @return	void
	 */
	protected function _rewind(int $position = 0): void
    {
		if ($position < 0)
		{
     ...
     *
     * @param integer $offset number of bytes to move forward
     * @return    int
     * @throws Nanodicom_Exception
     */
	protected function _forward(int $offset): int
    {
		$increment = $offset;
	 ...
	 *
	 * @param integer $starting_byte starting byte for reading
	 * @param mixed|null $length how many bytes to read
	 * @return	void
	 * @throws  Nanodicom_Exception
	 */
	protected function _read_file(int $starting_byte = 0, mixed $length = NULL): void
    {
		// Read contents
		...
		else
		{
			...
			{
				throw new Nanodicom_Exception('Error occurred. Reading :length from file :file at byte :byte got an error',
	 ...
	 * left to be read
	 *
	 * @param integer|null $length number of bytes to read or NULL
	 * @return 	boolean|string boolean when checking if there is still data or a binary string otherwise
	 * @throws  Nanodicom_Exception
	 */
	protected function _read(int $length = NULL): bool|string
    {
		if ($length === NULL)
		{
			// A quick way to check if file has reached its end or not.
			return !(($this->_current_pointer >= $this->_file_length));
		}
	...
	protected function _check_deflate(): void
    {
