...
class Nanodicom_Dictionary
{
	...
	public static array $transfer_syntaxes = [
	...
	protected static array $_loaded_dictionaries = [];
	 ...
	 *
	 * @param integer $group the group of the dictionary to load
	 * @param bool $force the vr_mode or to force to load the dictionary
	 * @return	void	 other times
	 */
	public static function load_dictionary(int $group, bool $force = FALSE): void
    {
		...
		// Let's load the dictionary if it was not loaded yet
		if (! isset(self::$_loaded_dictionaries[$group])  
			...
			AND file_exists('mmerlijn\\dicom\\Core\\dict\\'.sprintf('0x%04X',$group).'.php'))
		{
			// Load the dictionary
			...
			require_once('mmerlijn\\dicom\\Core\\dict\\'.sprintf('0x%04X',$group).'.php');
