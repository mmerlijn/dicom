...
class Dicom_Dumper extends Nanodicom {
	...
	// Default html output
	protected array $_output_html = array(
		'dataset_begin'	=> '<ul>',
		'dataset_end'	=> '</ul>',
		'item_begin'	=> '<li>',
		'item_end'		=> '</li>',
	...
	// Default echo output
	protected array $_output_echo = array(
		'dataset_begin'	=> '',
		'dataset_end'	=> '',
		'item_begin'	=> '',
		'item_end'		=> "\n",
	 ...
	 */
	public function dump($output = 'echo'): string
    {
		$this->parse();
		...
		foreach ($dataset as $group => $elements)
		{
			...
			foreach ($elements as $element => $indexes)
			{
				foreach ($indexes as $values)
				{
					...
					if ($group == Nanodicom_Core::ITEMS_GROUP AND in_array($element, array(Nanodicom_Core::ITEM_DELIMITER, Nanodicom_Core::SEQUENCE_DELIMITER)))
					{
	 ...
	 */
	protected function _dump_element($element, $space = ''): string
    {
	 ...
	 */
	protected function _print_element($element): string
    {
		$out = '';
