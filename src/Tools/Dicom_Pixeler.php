...
namespace mmerlijn\dicom\Tools;
use Gmagick;
use GmagickDraw;
use GmagickPixel;
use Imagick;
use ImagickDraw;
use ImagickPixel;
use mmerlijn\dicom\Core\Nanodicom_Core;
use mmerlijn\dicom\Core\Nanodicom_Exception;
...
class Dicom_Pixeler extends Nanodicom {
	...
	*/
	public static $reading_image_transfer_syntaxes = array(
		...
		Nanodicom_Core::IMPLICIT_VR_LITTLE_ENDIAN => array(
			'name' => 'Implicit VR Little Endian'
		),
		// Explicit VR Little Endian "1.2.840.10008.1.2.1"
		Nanodicom_Core::EXPLICIT_VR_LITTLE_ENDIAN => array(
			'name' => 'Explicit VR Little Endian'
		),
		// Explicit VT Big Endian "1.2.840.10008.1.2.2"
		Nanodicom_Core::EXPLICIT_VR_BIG_ENDIAN => array(
	...
	public function get_images(int $width = NULL, int $center = NULL): false|array
    {
		...
		if ( ! (!str_contains($window_center, "\\")))
		{
			$temp          = explode("\\", $window_center);
			$window_center = (int) $temp[0];
		}
		
		if ( ! (!str_contains($window_width, "\\")))
		{
		...
		if ($transfer_syntax_uid == '1.2.840.10008.1.2.4.50')
		{
			...
			foreach ($blob as $group => $elements)
			{
				...
				{
					if ($element == Nanodicom_Core::SEQUENCE_DELIMITER)
						continue;
					...
					foreach ($indexes as $values)
					{
						...
						else
						{
							...
							for ($i = 0; $i <= 16; $i++)
							{
								...
								$header[] = $this->{Nanodicom::$_read_int}(4, Nanodicom_Core::LITTLE_ENDIAN, 4, Nanodicom_Core::UNSIGNED, $chunk);
							}
							...
							foreach ($header as $starting_byte_of_segment)
							{
								...
								while ($bytes_count < $size_of_segment)
								{
									...
									$n = $this->{Nanodicom::$_read_int}(1, Nanodicom_Core::LITTLE_ENDIAN, 1, Nanodicom_Core::SIGNED, $n);
									// Add 1
									$bytes_count++;
	 ...
	 */
	public function write_image($image, $location, $format = 'jpg'): bool
    {
		switch (self::$driver)
		{
			case 'gd' :
     ...
     */
	protected function _read_image_blob($string): mixed
    {
		switch (self::$driver)
		{
	...
	protected function _read_gray($blob, $current_position, $bytes_to_read)
	{
		if ($this->_samples_per_pixel == 1)
		{
			...
			$gray = $this->{Nanodicom::$_read_int}($bytes_to_read, $this->_endian, $bytes_to_read, Nanodicom_Core::UNSIGNED, $chunk);
			$chunk = NULL;
		...
		else
		{
			...
			$chunk = substr($blob, $current_position, $bytes_to_read);
			return $this->{Nanodicom::$_read_int}($bytes_to_read, $this->_endian, $bytes_to_read, Nanodicom_Core::UNSIGNED, $chunk);
		}
	}
